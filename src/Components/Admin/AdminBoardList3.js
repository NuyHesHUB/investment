...
///// style /////
import {Wrap} from "./StyledAdminBoard3"
...
const AdminBoardList3 = () => {
  ...
  // const { status, title, categoryList } = boardData[index];
  ...
  const modifying2222222 = (index) => {
    if (modify.length === 0) {
      setIndex(index)
    } else if (modify === index/*  && window.confirm("수정testtest") */) {
      setModify('')
      axios.patch(`${baseURL}/v1/board/modify`, boardDataModi, { headers }).then((res)=> {
        navigate(`/admin/board_list`);
      }).catch((error)=> {
  ...
  console.log("boardData값", boardData)
